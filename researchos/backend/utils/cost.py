...
def estimate_run_cost(persona_count: int, question_count: int) -> tuple[float, int]:
    ...
    total_in  = agent1_in + agent2_in + (persona_count * agent3_in_per) + agent4_in
    total_out = agent1_out + agent2_out + (persona_count * agent3_out_per) + agent4_out
    total_cost = round(calculate_cost(total_in, total_out), 2)

    # Latency: Agent 3 runs all personas in parallel
